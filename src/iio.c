...
static void repair_broken_pixels_inplace(void *x, int n, int pd, int sz)
{
	...
	IIO_DEBUG("rpbi x=%p t=%p n=%d pd=%d sz=%d\n", x, t, n, pd, sz);
	memcpy(t, x, n * pd * sz);
...
// COS reader                                                               {{{2
// http://tandemx-science.dlr.de/pdfs/TX-GS-DD-3307_TerraSAR-X_Ground_Segment_Level_1b_Product_Format_Specification.pdf
static int read_beheaded_cos(struct iio_image *x,
		FILE *f, char *header, int nheader)
{
	IIO_DEBUG("got COS\n");
	char buf[255];
		...
		FILE *f, char *header, int nheader)
{
	...
	if (pd > 1)
		repair_broken_pixels_inplace(x->data, w*h, pd,
				iio_image_sample_size(x));
		...
		bool broken_pixels, bool endianness)
{
	...
	IIO_DEBUG("prbie w=%d h=%d pd=%d ss=%zu\n", w, h, pd, ss);
	memcpy(x->data, header_bytes + (char*)data, n);
	IIO_DEBUG("prbie xdata=%p nsamples=%zu, ss=%zu\n",x->data,nsamples,ss);
	if (endianness) {
		if (ss == 2)
	...
	IIO_DEBUG("prbie xdata=%p nsamples=%zu, ss=%zu\n",x->data,nsamples,ss);
	if (broken_pixels)
		repair_broken_pixels_inplace(x->data, w*h, pd, ss);
	return 0;
}
...
static int guess_format(FILE *f, char *buf, int *nbuf, int bufmax)
{
	...
	*nbuf = 0;
	...
	if (b[0]==40 && b[1]==159 && b[2]==119)
		return IIO_FORMAT_COS;
...
int read_beheaded_image(struct iio_image *x, FILE *f, char *h, int hn, int fmt)
{
	...
	case IIO_FORMAT_COS:   return read_beheaded_cos (x, f, h, hn);
